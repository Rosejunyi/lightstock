# scripts/calculate_indicators.py (凤凰涅槃版 - 根除数据丢失的逻辑缺陷)
import os
import sys
...
print(f"\n!!! RUNNING THE PHOENIX EDITION (Pagination: {PAGINATION_SIZE}) !!!\n")
...
def main():
    ...
    def calculate_all_indicators(group):
        ...
        # 1. 预先创建所有指标列，并用 np.nan 填充
        indicator_columns = ['ma10','ma20','ma50','ma60','ma150','high_52w','low_52w','volume_ma10','volume_ma30','volume_ma60','volume_ma90','macd_diff','macd_dea','rsi14','rs_raw']
        for col in indicator_columns:
            group[col] = np.nan

        # 2. 根据条件，安全地填充这些已经存在的列
        if len(group) < 30: return group # 数据太少，直接返回全是 NaN 的组
    ...
    records_to_upsert = []
    # 从函数内部移动到这里，作为统一的标准
    final_indicator_cols = ['ma10','ma20','ma50','ma60','ma150','high_52w','low_52w','volume_ma10','volume_ma30','volume_ma60','volume_ma90','macd_diff','macd_dea','rsi14','rs_rating']
    for index, row in today_indicators.iterrows():
        record = {'symbol': row['symbol'], 'date': row['date'].strftime('%Y-%m-%d')}
        for col in final_indicator_cols:
